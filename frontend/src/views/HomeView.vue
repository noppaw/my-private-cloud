  ...
  <div class="min-h-screen bg-gray-50 p-8 font-sans">
    ...
    <div v-else class="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      <a 
        ...
        :key="app.id" 
        :href="app.url" 
        class="block bg-white p-6 rounded-xl shadow-sm hover:shadow-md transition-shadow border border-gray-100 group text-center"
      >
        <div class="w-16 h-16 mx-auto bg-blue-50 text-blue-600 rounded-full flex items-center justify-center mb-4 group-hover:bg-blue-600 group-hover:text-white transition-colors">
          <i :class="['mdi', app.icon_name, 'text-3xl']"></i>
        </div>
